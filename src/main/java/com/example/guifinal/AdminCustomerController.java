...
public class AdminCustomerController implements Initializable {
    ...
    @FXML
    protected void onPromoteButtonClick(){
        selectedCustomer = customerTable.getSelectionModel().getSelectedItem();
        if(selectedCustomer != null){
            if(selectedCustomer instanceof VIPAccount){
                Alert alert = new Alert(Alert.AlertType.ERROR);
                alert.setTitle("Error");

                alert.setHeaderText(null);
                alert.setContentText("VIP Account can not be promoted anymore");

                alert.showAndWait();
            } else if (selectedCustomer instanceof RegularAccount) {
                if(selectedCustomer.getSuccessfullyReturn() < 5){
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");

                    alert.setHeaderText(null);
                    alert.setContentText("Regular Account can not be promoted unless the account have returned at least 5 times.");

                    alert.showAndWait();
                } else {
                    VIPAccount vipAccount = ((RegularAccount) selectedCustomer).promote();
                    Application.customerList.remove(selectedCustomer);
                    Application.customerList.add(vipAccount);
                }
            } else {
                if(selectedCustomer.getSuccessfullyReturn() < 3){
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");

                    alert.setHeaderText(null);
                    alert.setContentText("Guest Account can not be promoted unless the account have returned at least 3 times.");

                    alert.showAndWait();
                } else {
                    RegularAccount regularAccount = ((GuestAccount) selectedCustomer).promote();
                    Application.customerList.remove(selectedCustomer);
