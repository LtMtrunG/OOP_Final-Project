...
public class GuestAccount extends Customer {
    ...
    public static GuestAccount createAccount(){
        ...
        return new GuestAccount(ID, name, address, phone, username, password, 0);
    }
    ...
    public boolean rent(Item item) {
        ...
        if(this.getRentalList().size() == 2){System.out.print("A Guest Account can only rent maximum 2 items at a time!"); return false;}
    ...
    public String toString() {
        ...
        return "GuestAccount{" +
                "numberOfItems=" + this.getRentalList().size() +
                ", successfullyReturn=" + this.getSuccessfullyReturn() +
                '}';
    }
}
